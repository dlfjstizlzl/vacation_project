...
class Retrofit_loadingActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val return_tum = intent.getIntExtra("return_tum",0)
        Log.d("qr_num", qrnum.toString())
        Log.d("return_num", return_tum.toString())

        if (return_tum == 4321){
            val tumbler_num = db.collection("user").document(locked_email)
            val updates = hashMapOf<String, Any>(
                "tumbler_num" to FieldValue.delete(),
            )
            tumbler_num.update(updates).addOnCompleteListener {  }
            Log.d("fire_delete", "성공")
            var intent_nextActivity = Intent(this, ResultActivity::class.java)
            intent_nextActivity.putExtra("Success_Value",2)
